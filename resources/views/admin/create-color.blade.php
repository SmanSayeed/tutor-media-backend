<x-admin-layout title="Create Color">
  <!-- Page Title Starts -->
  ...
  <div class="space-y-6">
    <div class="card">
      <div class="card-body">
        <form class="space-y-6" action="{{ route('admin.colors.store') }}" method="POST">
          @csrf
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div class="space-y-2">
                ...
                <input type="text" id="name" name="name" class="input @error('name') is-invalid @enderror"
                  placeholder="e.g., Red, Blue, Green" value="{{ old('name') }}" required />
                @error('name')
              ...
              <div class="space-y-2">
                ...
                <input type="text" id="code" name="code" class="input @error('code') is-invalid @enderror"
                  placeholder="e.g., RED, BLUE" value="{{ old('code') }}" required />
                @error('code')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="space-y-2">
              ...
              <div class="flex gap-2">
                <input type="text" id="hex_code" name="hex_code" class="input @error('hex_code') is-invalid @enderror"
                  placeholder="#FF0000" value="{{ old('hex_code') }}" />
                <div id="color_preview" class="w-10 h-10 rounded border border-slate-300"
                  style="background-color: {{ old('hex_code', '#FF0000') }}"></div>
              </div>
              @error('hex_code')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
