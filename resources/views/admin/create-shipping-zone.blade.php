<x-admin-layout title="Create Shipping Zone">
  <!-- Page Title Starts -->
  <div class="mb-6 flex flex-col justify-between gap-y-1 sm:flex-row sm:gap-y-0">
    <h5>Create Shipping Zone</h5>
  ...
  <!-- Create Shipping Zone Form Starts -->
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div>
                ...
                <input type="text" id="division_name" name="division_name" class="form-input @error('division_name') is-invalid @enderror"
                       value="{{ old('division_name') }}" placeholder="e.g., Dhaka, Chittagong" required>
                @error('division_name')
              ...
              <div>
                ...
                <input type="text" id="zone_name" name="zone_name" class="form-input @error('zone_name') is-invalid @enderror"
                       value="{{ old('zone_name') }}" placeholder="e.g., Dhaka City, Comilla" required>
                @error('zone_name')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div>
                ...
                <input type="number" id="shipping_charge" name="shipping_charge" class="form-input @error('shipping_charge') is-invalid @enderror"
                       value="{{ old('shipping_charge') }}" placeholder="0.00" min="0" step="0.01">
              ...
              <div>
                ...
                <select id="status" name="status" class="form-select @error('status') is-invalid @enderror" required>
                  ...
                  <option value="deactive" {{ old('status') === 'deactive' ? 'selected' : '' }}>Inactive</option>
                </select>
                @error('status')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
  ...
  <!-- Create Shipping Zone Form Ends -->
