<x-admin-layout title="Child Category Details">
  <!-- Page Title Starts -->
  <div class="mb-6 flex flex-col justify-between gap-y-1 sm:flex-row sm:gap-y-0">
    ...
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item">
        <a href="{{ route('admin.child-categories.show', $childCategory->id) }}">Child Category Details</a>
      </li>
    </ol>
  </div>
  <!-- Page Title Ends -->

  <!-- Child Category Details Starts -->
  <div class="space-y-6">
    <!-- Child Category Information Card -->
    <div class="card">
      <div class="card-header">
        <h6 class="card-title">Child Category Information</h6>
      </div>
      <div class="card-body">
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div class="space-y-4">
            <div>
              <h3 class="text-xl font-semibold text-slate-800 dark:text-slate-200">{{ $childCategory->name }}</h3>
              <p class="text-sm text-slate-500 dark:text-slate-400">Slug: {{ $childCategory->slug }}</p>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <p class="text-sm text-slate-800 dark:text-slate-200">
                  {{ $childCategory->subcategory->category->name ?? 'N/A' }}</p>
              </div>
              <div>
                <span class="text-sm font-medium text-slate-600 dark:text-slate-400">Subcategory</span>
                <p class="text-sm text-slate-800 dark:text-slate-200">{{ $childCategory->subcategory->name ?? 'N/A' }}
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                <p class="text-sm text-slate-800 dark:text-slate-200">{{ $childCategory->sort_order }}</p>
              </div>
              <div>
                <span class="text-sm font-medium text-slate-600 dark:text-slate-400">Created Date</span>
                <p class="text-sm text-slate-800 dark:text-slate-200">{{ $childCategory->created_at->format('M d, Y') }}
                </p>
              </div>
              <div>
                <span class="text-sm font-medium text-slate-600 dark:text-slate-400">Products Count</span>
                <p class="text-sm text-slate-800 dark:text-slate-200">{{ $childCategory->products->count() }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        @if($childCategory->description)
          <div class="mt-6">
            ...
            <p class="text-sm text-slate-700 dark:text-slate-300">{{ $childCategory->description }}</p>
          </div>
        @endif
      </div>
    </div>
    ...
    @if($childCategory->meta_title || $childCategory->meta_description)
      <div class="card">
        <div class="card-header">
    ...
    <!-- Products in this Child Category -->
    @if($childCategory->products->count() > 0)
      <div class="card">
        <div class="card-header">
          <h6 class="card-title">Products in this Child Category ({{ $childCategory->products->count() }})</h6>
        </div>
        <div class="card-body">
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            @foreach($childCategory->products->take(6) as $product)
            ...
            @endforeach
            @if($childCategory->products->count() > 6)
              <div class="flex items-center justify-center rounded-lg border border-slate-200 p-3 dark:border-slate-700">
                <span class="text-sm text-slate-600 dark:text-slate-400">+{{ $childCategory->products->count() - 6 }} more
                  products</span>
              </div>
            @endif
          </div>
    ...
    <div class="flex flex-col justify-end gap-3 sm:flex-row ">
      <a href="{{ route('admin.child-categories.index') }}" class="btn btn-secondary">
        ...
        <span>Back to Child Categories</span>
      </a>
      <a href="{{ route('admin.child-categories.edit', $childCategory) }}" class="btn btn-primary">
        <i data-feather="edit" class="h-4 w-4"></i>
        <span>Edit Child Category</span>
      </a>
    </div>
  </div>
  <!-- Child Category Details Ends -->
</x-admin-layout>